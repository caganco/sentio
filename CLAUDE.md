...
- **Her spec kendi branch'inde geliştirilir:** `feature/d{N}-{kisa-isim}`
  - Örnek: `feature/d117-pyproject-merge`, `feature/d113-kap-signature-fix`
  - `{N}` spec numarası (D-XXX) ile eşleşir, `{kisa-isim}` kebab-case.
...
- **Conflict → Orchestrator'a eskalasyon.** Builder, `master` ile çakışan bir
  ...
  ve çözüm kararını ona bırakır. (Spec enforcement prensibinin uzantısı.)
...
Yeni spec vermeden önce ETKİLENEN DOSYALAR listesi kontrol edilir.
Aktif speclerle aynı dosyaya dokunan yeni spec → sırayla ver, paralel değil.
İhlal: aynı dosyaya 2 Builder aynı anda dokunursa shared working tree çakışır.

## BUILDER PR KURALI (Zorunlu)

Her spec tamamlandığında Builder şunu söyler:
...
the maintainer sadece merge komutunu çalıştırır:
  gh pr merge XX --merge

Merge conflict çıkarsa Builder çözer, the maintainer görmez.
the maintainer terminal açmaz — sadece merge komutunu yapıştırır.
...
- **Basit spec** (1–3 dosya, izole değişiklik): CI'a bırak, Builder çalıştırmaz.
  CI merge blocker — yeşil olmadan merge edilemez, güvenlik ağı var.
- **Karmaşık spec** (5+ dosya, cross-module, mimari değişiklik): Builder PR öncesi çalıştırır:
  ```powershell
  python -m pytest tests/ -q --tb=short
  ```
...
- **Her yeni SPEC/spec, ilgili RR-XXX raporlarına `§section_number` ile referans vermek zorundadır.**
...
- **Yeni RR dosyası eklendiğinde RESEARCH_REGISTRY.md ZORUNLU güncellenir.**
  ...
  - Status başlangıçta `⏳ Uygulanmadı`; spece dönüştüğünde `✅ Applied` + D-XXX yazılır.
...
5. **[FEATURE_INDEX.md](FEATURE_INDEX.md)** — aktif feature'lar, faz durumu, neye dokunma özeti
   (Büyük spec vermeden önce ilgili `docs/features/FEATURE_NAME.md` de okunur)
