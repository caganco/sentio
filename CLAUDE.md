...
## Yerel Geliştirme Araçları (Cagan'ın makinesinde global kurulu)

Bu araçlar repo dışında, makine seviyesinde kalıcı olarak kuruludur. Tüm clone'lardan
(builder1/2/3) ve ana repodan çalışır. Unutulmaması için buraya yazıldı.

### `pullall` — 4 repoyu tek komutla `master`'a güncelle
- **Konum:** `C:\Users\cagan\bin\pullall.ps1` (+ `pullall.cmd` shim). `bin` User PATH'tedir.
- **Kullanım:** herhangi bir cmd/pwsh penceresinde → `pullall`
- **Davranış:** ana repo (`bist-trading-system`) + `bist-clone-builder1/2/3` clone'larını
...
- **cmd:** `C:\Users\cagan\bin\git-guard.cmd` + cmd AutoRun makrosu
...
`C:\Users\cagan\OneDrive\Documents` ölü bir OneDrive bulut placeholder'ıdır (OneDrive
