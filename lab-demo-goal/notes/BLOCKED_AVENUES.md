...
gerektiriyor, ya da Cagan'in MANUEL ag/auth islemi gerektiriyor. Hicbiri otonom-cekilemez
(repo READ-ONLY + ag-pull Cagan-onayina kapali). Burasi tek-bakista "neden bakilamadi + ne gerekir"
kaydidir. Oncelik-gerekce: [FORWARD_DATA_SPEC.md]; go/no-go karti: [FORWARD_DECISION_CARD.md].

Etiket: [DATA-GAP] = veri-turu bizde yok/uretilemez. [MANUEL-AUTH] = Cagan ag/login/onay-fetch lazim.
...
- NE GEREKIR: `kap_historical_fetcher` (schema'da `publication_date` ZATEN var ama cache'ler BOS) ile
  ...
  fiscal_year, quarter, sue]. Yeni-scraper DEGIL; mevcut-fetcher'i Cagan-onayli kosturmak.
...
- NE GEREKIR: TEFAS'tan yeni-build cekim-hatti (ag-fetch + parse + panel-insa). Cagan-onayli kosum.
...
- NE GEREKIR (gercek-run): tarihsel haber-corpus (cok-yillik) + her-haber-gun duyarlilik-skorlama -> ag-fetch +
  saklama + NLP-skorlama hatti (Cagan-onayli). Offline-uretilemez. Haber-akisi cogu KAP-ifsa -> #1 PEAD ile ortusur.
...
- NE GEREKIR (gercek-run): tarihsel KAP-ifsa tam-metin arsivi (gun-damgali) + NLP-pipeline. Ag-fetch, Cagan-onayli.
...
- L21 KOSULDU (open-interest cross-sectional): SSF (tek-hisse-futures) ACIK POZISYON kullanilarak
  ...
  VIOP-OI-XS-NOT-TRADEABLE. Reversal AYRI gelecek-track adayi olarak loglandi (Cagan-mandasi + taze Stage-0 +
...
- INDEX-BASIS/TERIM-YAPISI ekseni L22'de KOSULDU -> VIOP-TS-FEASIBILITY-BLOCKED: BIST30 index-futures
  ...
  Cagan-kapili ILERI-aday loglandi. [L22_viop_term_structure_REPORT.md]
...
## OZET (Cagan icin) -- 2026-06-04 guncel
