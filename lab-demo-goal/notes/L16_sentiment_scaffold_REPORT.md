...
SCRAPER DEGIL, network YOK, yeni-edge YOK. Spec ACIKCA SENTIMENT istedi; offline tek sentiment/haber
...
- Verdict-kurali: keep-bar gecerse TRADEABLE-EDGE (deploy-aday -> the maintainer); yoksa SENTIMENT-NOT-TRADEABLE.
...
- **Veri-gap sayisal**: snapshot 6-sembol/1-ay -> backtest IMKANSIZ; gercek-test tarihsel gun-damgali
  haber-corpus + duyarlilik-skorlama hatti (the maintainer-onayli ag-fetch) ister. Offline-uretilemez.
...
Specin SENTIMENT avenusu L11-forward-scaffold formuna kristalize edildi: test donmus ve pipeline-dogru,
gercek gun-damgali haber/sentiment fetch'inden (the maintainer-kapili) tek-adim uzakta. Sentetik PASS yalniz
pipeline-dogrulugu + look-ahead-guvenligi kanitlar; gercek BIST-edge'i ag-fetch'in the maintainer-onayina KAPILI.
