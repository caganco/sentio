"""Behavior tests for the D-204 realistic per-stock cost model. Synthetic, no network.
...
adv -> nan), the RR-015 tier half-spread monotonicity, and the round-trip combine rule
(round_trip = 2*(spread+impact)+commission; tier fallback when Roll is 0/undefined).
"""
...
# ---------------------------------------------------------------------------
# RR-015 tier half-spread floor
# ---------------------------------------------------------------------------
def test_tier_spread_floor_monotone_mega_to_micro():
    mega = rc.tier_spread_floor(th.D204_TIER_MEGA_ADV_TL + 1)
    large = rc.tier_spread_floor(th.D204_TIER_LARGE_ADV_TL + 1)
    mid = rc.tier_spread_floor(th.D204_TIER_MID_ADV_TL + 1)
    micro = rc.tier_spread_floor(th.D204_TIER_MID_ADV_TL - 1)
    assert mega < large < mid < micro
    assert mega == pytest.approx(th.D204_TIER_MEGA_HALF_SPREAD)
    assert micro == pytest.approx(th.D204_TIER_MICRO_HALF_SPREAD)


def test_tier_spread_floor_nonpositive_adv_is_micro():
    assert rc.tier_spread_floor(0.0) == pytest.approx(th.D204_TIER_MICRO_HALF_SPREAD)
    assert rc.tier_spread_floor(float("nan")) == pytest.approx(th.D204_TIER_MICRO_HALF_SPREAD)


# ---------------------------------------------------------------------------
# Round-trip combine rule
# ---------------------------------------------------------------------------
def test_combine_round_trip_is_twice_spread_plus_impact():
    d = rc.combine_round_trip(roll_spread=0.01, kyle_impact_val=0.002,
                              tier_spread=0.005, commission=0.0)
    assert d["round_trip_roll"] == pytest.approx(2.0 * (0.01 + 0.002))
...
def test_combine_round_trip_falls_back_to_tier_when_roll_zero():
    # Roll undefined/0 (serial cov >= 0) -> ROLL leg uses the tier floor + flags it.
...
def test_combine_round_trip_impact_falls_back_to_tier_when_undefined():
    d = rc.combine_round_trip(roll_spread=0.01, kyle_impact_val=float("nan"),
                              tier_spread=0.005, commission=0.0)
    # impact undefined -> impact_eff = tier floor (0.005)
    assert d["round_trip_roll"] == pytest.approx(2.0 * (0.01 + 0.005))
...
def test_round_trip_cost_end_to_end_keys():
    ...
    out = rc.round_trip_cost(close, adv=1_000_000.0, order_value=20_000.0,
                             window=21, lambda_kyle=1.0)
    for k in ("roll_spread", "kyle_impact", "tier_spread", "roll_is_zero",
              "round_trip_roll", "round_trip_tier"):
        assert k in out
