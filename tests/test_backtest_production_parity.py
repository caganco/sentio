...
def _bt_composite(tech: float, macro: float) -> float:
    """Backtest composite formula: L3/L4/L5 = 50.0 neutral stub.

    backtest/engine.py _compute_composite() satir 217-226 ile ozdes.
    D-154: L6 risk removed from composite; function signature updated.
    """
...
def _prod_composite(
    ...
    """Production composite formula: full 5-layer weighted sum.

    src/signals/engine.py _compute_weighted_sum() ile ozdes yapi.
    D-154: L6 risk removed from composite; parameter dropped.
    """
    return max(0.0, min(100.0,
        tech * MASTER_WEIGHTS["technical"]
        + macro * MASTER_WEIGHTS["macro"]
...
class TestBacktestNeutralStubMath:
    ...
    def test_neutral_stub_weight_fraction(self):
        """L3/L4/L5 toplam agirligi ~%53.6 (D-154 renorm) — divergence kapasite.

        D-154: risk/L6 removed; remaining 5 layers renorm /0.97.
        kap=0.30/0.97 + sentiment=0.12/0.97 + smart_money=0.10/0.97 = 0.52/0.97 ~0.5361
        """
        neutral_weight = (
            MASTER_WEIGHTS["kap"]
            + MASTER_WEIGHTS["sentiment"]
        ...
        # D-154: renormalized from 0.52 to 0.52/0.97
        assert neutral_weight == pytest.approx(round(0.52 / 0.97, 10), abs=0.001)

    def test_neutral_stub_contribution_at_50(self):
        """50.0 neutral stub -> composite'e ~26.8 puan sabit katki (D-154 renorm).

        D-154: 50.0 * (0.52/0.97) ~ 26.804 puan.
        """
        contribution = 50.0 * (
            MASTER_WEIGHTS["kap"]
            + MASTER_WEIGHTS["sentiment"]
        ...
        # D-154: renormalized from 26.0 to 50.0 * (0.52/0.97) ~ 26.804
        assert contribution == pytest.approx(50.0 * round(0.52 / 0.97, 10), abs=0.001)

    def test_max_positive_divergence(self):
        """L3/L4/L5 = 100 iken production - backtest maksimum fark ~26.8 puan (D-154).

        D-154: (100 - 50) * (0.52/0.97) ~ 26.804 puan maksimum divergence.
        """
        max_div = (100.0 - 50.0) * (
            MASTER_WEIGHTS["kap"]
            + MASTER_WEIGHTS["sentiment"]
        ...
        # D-154: renormalized from 26.0 to (100-50) * (0.52/0.97) ~ 26.804
        assert max_div == pytest.approx(50.0 * round(0.52 / 0.97, 10), abs=0.001)

    def test_zero_divergence_when_l3_l4_l5_neutral(self):
        """L3/L4/L5 = 50.0 iken backtest composite = production composite."""
        tech, macro = 65.0, 55.0
        bt = _bt_composite(tech, macro)
        prod = _prod_composite(tech, macro, 50.0, 50.0, 50.0)
        assert bt == pytest.approx(prod, rel=1e-6)

    def test_divergence_positive_for_strong_l3_l4_l5(self):
        """Guclu KAP/Sentiment/SmartMoney: production composite > backtest composite."""
        tech, macro = 65.0, 55.0
        bt = _bt_composite(tech, macro)
        prod = _prod_composite(
            tech, macro, kap=80.0, sentiment=72.0, smart_money=75.0
        )
        assert prod > bt, (
            f"Guclu L3/L4/L5: production ({prod:.2f}) > backtest ({bt:.2f}) bekleniyor"
        )

    def test_divergence_negative_for_weak_l3_l4_l5(self):
        """Zayif KAP/Sentiment/SmartMoney: production composite < backtest composite."""
        tech, macro = 65.0, 55.0
        bt = _bt_composite(tech, macro)
        prod = _prod_composite(
            tech, macro, kap=25.0, sentiment=30.0, smart_money=20.0
        )
        assert prod < bt, (
            f"Zayif L3/L4/L5: production ({prod:.2f}) < backtest ({bt:.2f}) bekleniyor"
        )
...
class TestDivergenceLog:
    ...
    def test_divergence_scenarios_logged(self):
        ...
        tech_fixed, macro_fixed = 65.0, 55.0

        for name, l3, l4, l5 in SCENARIOS:
            bt = _bt_composite(tech_fixed, macro_fixed)
            prod = _prod_composite(tech_fixed, macro_fixed, l3, l4, l5)
            delta = prod - bt
            logger.info(
        ...
        # Invariant: S3 Neutral senaryosunda delta = 0 olmali
        bt_n = _bt_composite(tech_fixed, macro_fixed)
        prod_n = _prod_composite(tech_fixed, macro_fixed, 50.0, 50.0, 50.0)
