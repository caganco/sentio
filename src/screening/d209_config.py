...
edge-arastirma graveyard (0bp daily NW-t=2.57), but it was eliminated there under a FLAT
...
HONEST EXPECTATION (declared BEFORE results): the frozen edge-arastirma h2b_runup_basket.json
...
edge-arastirma H2/H2b signal (detection + book) bit-for-bit so the run-up factor cannot drift.

Two FROZEN edge-arastirma variants are reported (NO "best" selected):
  V1 daily-churn basket (edge-arastirma h2b_runup_basket.py BIREBIR): held on day t iff ex-date
...
# ---------------------------------------------------------------------------
# Frozen H2b/H2 signal geometry -- PORTED from the edge-arastirma H2 lab (BIREBIR, NO drift).
# ---------------------------------------------------------------------------
...
# Frozen-detection reproduction guard (edge-arastirma H2 lab: ~1108 events / 265 symbols on the
...
# Legacy FLAT cost columns kept ONLY for the edge-arastirma-comparable context table (NOT the
...
D209_CANDIDATE_LABEL = (
    ...
    "ilk-resmi-olcum (N<=3), edge-arastirma-FLAT-eleme sonrasi D-207-duzeltilmis-maliyet re-test")
