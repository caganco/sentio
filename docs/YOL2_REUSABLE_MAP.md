...
| Scheduler/otomatik çalışma | ❌ BAĞLI DEĞİL — Cagan ~haftalık `scripts/event_forward_capture.py` ile elle |
...
**Öneri:** `data/event_logs/` dizinini başlatmak için Cagan'ın bir kez scripti elle çalıştırması gerekiyor. İlk çalışma sonrası haftalık rutin oluşturulabilir.
