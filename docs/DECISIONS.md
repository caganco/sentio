...
OLCER + onerir; Yol-1 lab'a terfi O+Cagan karari.
...
2. **Cagan onaylar:** Orchestrator değerlendirmesini Cagan inceler ve
   açık onay verir. Sessiz kalma onay sayılmaz.
3. **Ayrı direktif ile thresholds.py güncellenir:** Onaylanan değerler
   ayrı bir direktif (D-XXX) kapsamında `MASTER_WEIGHTS` bloğuna yazılır.
...
4. **PR → CI → merge:** Standart branch workflow (CLAUDE.md) uygulanır.
   Green CI + Cagan merge onayı olmadan üretim değişmez.
