...
**Tarih:** 30 Mayıs 2026
**Statü:** Yön kararı (the project). Tam ARCHITECTURE.md v2.0 revizyonu Architect'e devredilecek.
...
- **Sorumlulukların ayrılması:** sert kısıtlar=deterministik, tarama=kantitatif, sentez=LLM(öneri), nihai karar=the maintainer
...
- **Kullanıcı nihai karar verici** (Katman A öneri → the maintainer seçer → Katman C icra)
...
Katman A = 
  SÜZGEÇ (faktör/temel — GETİRİ MOTORU DEĞİL, izleme listesi daraltıcı):
    ...
    → "hangi hisselere bak" — the maintainer'ın zayıf olduğu finansal analizi otomatikler
  +
  ANA MOTOR (zaman-serisi trend-başı yakalama):
    ...
    - TREND BAŞLANGICI / düşük-riskli giriş (destek-direnç flip, 
      konsolidasyon-retest) — the maintainer'ın doğal tarzı
...
- the maintainer tarzı örtüşmesi: destek-direnç flip + konsolidasyon-retest + parabolikten-kaçınma + iyi-finansal-aşağıda-kalmış = raporun önerisiyle birebir

---

## KATMAN A SİNYAL MANTIĞI — CONFLUENCE (the maintainer içgüdüsü)
...
```
Confluence = signal confirmation. the maintainer'ın "iki yönden tümdengelim" (teknik beğendiğime yabancı akışı da varsa öncelik / yabancı akışı olana teknik bakışı) içgüdüsünün sistemsel karşılığı.
...
## BAŞARI KISTASI — PER-TRADE (the maintainer psikolojisi + gerçeklik)

**v1.2'deki "yıllık portföy getirisi" çerçevesi → per-trade expectancy + fırsat sıklığı:**
- Her işlem, tuttuğu sürenin faiz fırsat-maliyetini ASİMETRİK yensin (the maintainer hedefi: ~%10-15 / birkaç hafta)
...
Birkaç hafta–birkaç ay (the maintainer içgüdüsü ✓, RR-038 onayı). Çok-kısa-vade YASAK (reversal + maliyet erozyonu).
...
- value (MaliTablo): getiri motoru değil, KALİTE SÜZGECİ (the maintainer'ın "iyi finansal" filtresi)
