# SPEC_PIVOT_ARCHITECTURE_1 — Strangler Refactor Builder Specleri

**Versiyon:** 1.0
**Tarih:** 29 Mayıs 2026 — Session #8
**Status:** Architect SPEC — onaylı, Builder speclerine bölünmüş yol haritası
...
> Bu SPEC, ARCHITECTURE v1.2'yi **uygular, sorgulamaz.** S1-S5 ve EK KARAR 1-3 kapalı kararlardır. SPEC kod yazmaz — hangi modül, hangi sırayla, hangi test, hangi başarı kriteri sorularına yanıt verir. Her spec tek tek build edilebilir bir Builder görevidir.
...
ARCHITECTURE v1.2'nin dört-katmanlı tarama-öncelikli quantamental mimarisini (A tarama / B LLM asistan / C icra / D makro şalter), **in-place strangler refactor** ile inşa edilecek **sıralı Builder speclerine** böler. Strangler ilkesi: yeni modüller mevcut yapının yanına kurulur, eski linear-additive çekirdek EN SON budanır (eski çalışırken).
...
- Kod yazmaz — yol haritası + spec tanımı; implementasyon Builder'ın.

### 0.3 Referans yöntemi

Her spec, dokunulacak/dokunulmayacak dosyaları AUDIT'ten **`dosya:satır`** referansıyla verir. Doğrulama geçitleri ARCHITECTURE §7.1'e, neye-dokunma invariant'ları §9'a (12 invariant) bağlıdır.

---

## §1. KRİTİK SIRALAMA KARARI (DEC-039)

**İLK spec TAM TARAMA SİSTEMİ DEĞİL — Faz 0 = Faktör IC Validasyon Harness'ı.**
...
| Faz | Spec | Katman | Bağımlılık | Hedef geçit |
|-----|----------|--------|------------|-------------|
...
Her spec şu şablonu izler: **Amaç · Bağımlılık · Dokunulacak (file:line) · DOKUNULMAYACAK · Başarı kriteri · Test gereksinimi · Hedef geçit · Korunan invariant'lar.**
