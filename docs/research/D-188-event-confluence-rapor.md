...
**Kritik içgörü (Cagan):** token uzun süre gelmeyebilir → forward sistem token-darboğazını **bypass eder** (auth-suz feed bugün çalışıyor, §5d).
...
- ⏳ **Bloke (backtest):** E1 tarihsel XBRL (MKK_VYK_TOKEN); E2 endeks-dahil kaynağı (yok → **ayrı direktif**); E3 ODA tarihsel (token). yfinance-fallback duyuru-tarihi → orta look-ahead riski (tercih edilmez).
...
- **`NRR-001-SCREENING.md`** içerik-ilgili: §3 "RS XU100-relative, nominal değil" + look-ahead/survivorship disiplini → XU100-relative + reel-sürpriz + look-ahead seçimlerimize **gerçek dayanak**. **AMA UNTRACKED** (`git status: ??`) → kayıt-altı kanıt değil; PR'a **dahil edilmedi**; commit kararı **Cagan/Orchestrator**'a (başkasının untracked araştırmasını sessiz commit'leme — shared-tree dersi).
...
- E2/E3 backtest **ayrı veri-sağlama direktifi** ister.
...
Bu program **ÖLÇMEYE HAZIRLAR**; verdict (backtest token/veri **VEYA** forward örneklem sonrası) **O+Cagan** kararı. Builder **ÖNERİR, atfetmez.**

**Öneri:** (1) Forward recorder'ı **bu hafta manuel** başlat (auth-suz feed çalışıyor, token gerekmez) → temiz örneklem biriksin. (2) Backtest için MKK_VYK_TOKEN sağlanırsa E1 tarihsel açılır; sağlanmazsa forward tek-başına yeterli (daha temiz). (3) E2 endeks-dahil için ayrı veri-sağlama direktifi. **"Edge yok" sonucu da geçerli** → swing'in son alt-kümesi de kapanırsa Yol 2'ye netçe çapalanılır.
