...
- **Dependent (LOCK):** XU100.IS fiyat-only (temettu YOK; snapshot-meta: hisse ~%2-4/yil dezavantajli;
  XU100-total-return yerel YOK -> fiyat-getiri direktif-izinli fallback). Aylik nominal
  `r_nom(t) = idx(t-sonu)/idx(t-1-sonu) - 1`; TL-reel = `r_nom(t) - infl(t)` (TUFE MoM cikarmasi,
  direktif-literal; deflasyon ZORUNLU).
- **Pencere (LOCK, Orchestrator-onayli Option-1, 2026-06-04):** PRIMARY 2019-01..2026-04 (87 forward-ay).
  Direktif 2010-01..2026-04 PRIMARY demisti; DATA-FACT (D-210): tek temiz LOKAL XU100 (exposure_d187_xu100)
...
- **Istatistik (LOCK):** OLS slope + Newey-West HAC t, lag=6 (direktif lag>=6). Seri lag-2 predictor
...
sonuclardan ONCE commit'lendi (anti-post-hoc guard). Deployment ayri bir O+Cagan kararidir; bu
