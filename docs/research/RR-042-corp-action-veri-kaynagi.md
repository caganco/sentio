...
**Karar (DEC-039 sınıfı, O+Cagan):** Hibrit — fiyat/PIT-üyelik/survivorship/otoriter-ex-date
...
tolerans-DIŞI kalan gerçek-bedelli için **KAP/MKK_VYK structured CA formu** (bütçeli, Cagan-onaylı).
...
- `MKK_VYK_BASE_URL` (apigwdev→Basic auth) + `MKK_VYK_TOKEN` env gerekir (builder'da yok → Cagan-env).
...
- **Sonuç:** sadece delisted-bedelli kalıntısı için Cagan-manuel academic-erişim opsiyonu.
...
**Kısıt (Cagan, verbatim):** "whitelist değil ip'miz — sınırlı sayıda istek." → kör pagination yasak.
...
- DAGHL/PEHOL/QNBFL için spesifik tarihsel CA bildirimini bulmak, tarih-filtresiz endpoint'te
  kör ikili-arama gerektiriyor (yukarıda KAYNAK-1). ~30+ çağrı sonrası, Cagan'ın "sınırlı istek"
...
| Yalnız price-implied tolerans-dışı (gerçek-bedelli) | **KAP/MKK_VYK CA formu (bütçeli, Cagan-onay)** | ≤birkaç sembol |
...
## Cagan-manuel adımlar (engel-değil, paralel)
...
- **D-20Y (Yol-2, opsiyonel, bütçeli):** sadece Yol-1'de "gerçek-bedelli" flag kalan + reachable
  ...
  (örn. 40), Cagan-onaylı tek koşu.** HALKS/IDEAS/ITTFH (2019, members'ta yok) erişilemez kalır →
