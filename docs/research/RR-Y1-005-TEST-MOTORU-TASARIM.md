...
**Statü:** TASARIM (Faz-0, kâğıt). Builder-recon (Faz-1) ve build (Faz-2) öncesi.
...
### 4.3 Rejim — manuel-girdi, motor-tespiti-değil (the maintainer-düzeltmesi, S#14)
...
**Dürüst-kalıntı (masada):** rejime-özgü doğrulanmış edge'in canlı-değeri, manuel-rejim-çağrının doğruluğuna koşulludur. Bu the maintainer-üstlenimi; motor-iddiası-değil.
...
| 4 | Embargo (h) | Builder-ölçer (recon) | otokorelasyon-sönüm-uzunluğu |
...
DISC-6 (koda-göm): Stage-0-freeze + purge/embargo + çoklu-test-düzeltmesi motorun-içinde-otomatik; Orchestrator-freni-değil altyapı-garantisi.
...
**Recon (Builder) çözer — codebase/veri-fizibilite:**
