...
Tasarım kâğıtta donmuş ama varsayımları veriyle/codebase'le henüz-doğrulanmadı. Recon-bulguları spec'i değiştirebilir; spec-kararı Orchestrator+Çağan'ın (ARCHITECTURE > Builder). Sen önce zemini-haritalayacaksın; build sonraki-faz.
...
## C. BUILDER ALT-TEŞHİS YETKİSİ (Çağan-direktifi)
...
**Sınır:** teşhis/öneri serbest; **mimari/spec-kararını VERME** (onu Orchestrator+Çağan donar). Sen "şu-sorun-var, şu-seçenekler-var, şunu-öneririm" dersin; "şöyle-yaptım" demezsin.
