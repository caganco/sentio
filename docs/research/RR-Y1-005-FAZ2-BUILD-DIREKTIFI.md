...
**Donmuş-girdi:** `RR-Y1-005-TEST-MOTORU-TASARIM.md v0.2` — SPEC. Çelişki-önceliği: TASARIM > bu-spec > Builder-tercihi. Tasarımdan-sapma gerekiyorsa DURUR, gerekçeyle Orchestrator'a sorarsın (kör-sapma-yok).
...
- `RESEARCH_REGISTRY.md` satır-önerisi (commit the maintainer-manuel).
...
## EK — ÇATAL-KARARLARI (S#14, Orchestrator+the maintainer; build-specine bağlayıcı)
...
**Hand-off (Builder'a üç-dosya, sırayla):** tasarım v0.2 (bağlam) → math-spec v1.1 (formel-çekirdek) → bu build-speci (görev + bu-EK).
