...
**Hedef Okuyucu:** Şu anki Cagan (Phase 5 sonu, RR-018 backtest framework var). Bu özetin amacı, "RR-019'u bugün uygulamam gerekir mi?" sorusuna net cevap vermek.
...
- **Öneri:** "7-agent ideal ama 2-LLM pratiktir." TradingAgents (Xiao et al. 2024, arXiv:2412.20138) v7 mimarisi (7 specialist agent + Bull/Bear debate + Risk team) Cagan bütçesini ($20/ay) aşar; pratik MVP **2-LLM jüri**: Claude Opus 4.7 (Bull advocate) + GPT-5.4 (Bear advocate), disagreement-aware position sizing ile.
...
**Hedef Okuyucu:** Gelecekteki Cagan / Builder. Bu bölüm Multi-LLM yaklaşımının niçin alpha ürettiğine dair literatürün özeti — implementation kararı vermeden önce okunması zorunlu.
...
- **Cagan + 1 finans uzmanı** (junior quant pozisyon ağı, mentorlar veya BÜMK alumni network) **bağımsız etiketleme** yapar. Etiket uzayı: `{BUY-STRONG, BUY-MEDIUM, HOLD, SELL-MEDIUM, SELL-STRONG, IRRELEVANT}`.
...
**Hedef Okuyucu:** Phase 6+ Cagan/Orchestrator. 7-agent'a "ne zaman geçilir" kararı için referans.
...
- Final karar Cagan'da kalır; LLM jüri sadece sayısal `llm_agreement_scalar`'ı raporlar.
- Skip durumunda Cagan manuel override edebilir ("LLM'ler çelişti ama ben yine de alacağım" — DEC-010 izin verir).

---

## 7. Maliyet Analizi (Detaylı)

**Hedef Okuyucu:** Cagan. Bütçe kararı için.
...
**Cagan bütçe ($20/ay) tüm senaryolarda karşılanabilir** — kritik nüans: cache'i agresif uygulamak ve büyük input context'i statik tutmak şart. Cache uygulanmazsa 2-LLM bile $15–20/ay seviyesine çıkar.

---

## 8. RR-011 ile Sinerji — Birleşik Token Bütçesi

**Hedef Okuyucu:** Builder/Cagan. RR-011 ve RR-019 birlikte çalıştığında bütçeyi yönetmek.

### 8.1 Bileşenler

**L4 Sentiment (RR-011 hibrit, D-124 aktif):**
- Tier 1 lexicon: $0, her gün ~2.000 haber işliyor.
- Tier 2 LLM (Claude Haiku 4.5): sadece "muğlak" haberler için. ~$10/ay baseline (Cagan ölçümü).
...
- Ham: $13.60–$23 (yüksek tarafta Cagan $20 hedefini aşar)
- Cached: $11–$14 (rahat)
...
- v2 (2-LLM) + RR-011 + optimizasyon: **$13–16/ay** → Cagan hedefini karşılar ✓
...
**Hedef Okuyucu:** Cagan. Operasyonel risk ve mitigasyon.
...
| **LLM jüri overconfidence (her ikisi de yanlış ama agree)** | Orta | Yüksek | RR-019 alpha kaynağı **değil**; RR-010/012/017 ana sinyal, RR-019 sadece sizing modifier. LLM yüksek conviction ama L1/L2/L5 ile çelişiyorsa Cagan diskresyoner override yapar |

---

## 10. BIST 2024–2026 Sektör Pratiği

**Hedef Okuyucu:** Cagan. Türkiye'de "AI-assisted trading" ne kadar yaygın, mevzuat ve maliyet farkındalığı.
...
- Cagan'ın 2026'da RR-019 implement ederken: model knowledge cutoff yeni rejimi (50% faiz) biliyor, **eski paradigmaya (%8.5 faiz) takılı kalmaz**.
...
**Hedef Okuyucu:** Phase 6+ Cagan/Builder. "AUM 500K TL aşıldığında ne yapacağım."
...
## 13. Cagan Portföy Analizi

**Hedef Okuyucu:** Cagan. Mevcut 3 pozisyon (TTKOM, KCHOL, ENERY) örnek üzerinden LLM jüri ne öğretir.
...
- `llm_agreement_scalar ≈ 0.5` → yarım pozisyon. RR-013 alphası burada **doğrulanmaz** (LLM çelişiyor) → Cagan diskresyoner: RR-013 ana sinyal, LLM modifier; pozisyon büyük tutulur ama LLM uyarı not edilir.
...
### 13.3 "Cagan'ın Diskresyoner Kararına" Katkı
...
3. Cagan her sabah daily report'ta bunu okur, RR-010/012/017 sinyalleriyle çapraz kontrol eder, **final pozisyon kararını verir**.

LLM jüri Cagan'ın **mental tartışma partneri**dir, otomat değil. Hatasız kalibrasyon hedefi değildir — hedef "Cagan'ın gözden kaçırdığı karşı argümanı görünür kılmak"tır.
...
class StrategistV2Report:
    ...
    advisory_text Cagan'ın okuduğu Markdown.
    """
...
**Hedef Okuyucu:** Builder/araştırmacı Cagan. Her madde 2–3 cümle özet + DOI/arXiv URL.
...
**Hedef Okuyucu:** Cagan. Bu raporun ne yapamadığı ve ne zaman geçersizleşeceği.
...
5. **AI black box / interpretability.** LLM'in nasıl "BUY-STRONG" dediği opak. Reasoning field açıklama sağlar ama post-hoc rationalization olabilir (LLM her zaman doğru sebep vermez). Cagan ve Builder LLM çıktısına **asla sebep gösterilen veriyi doğrulamadan güvenmemelidir**.
