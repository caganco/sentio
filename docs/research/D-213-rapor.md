...
**COZUM-LOCK (Cagan, sonuc-oncesi tasarim-secimi -- post-hoc DEGIL):** nakit-bacak TLREF DEGIL;
...
- **Dependent (LOCK, D-211 ile AYNI):** XU100.IS fiyat-only (temettu YOK; fiyat-getiri fallback).
  `r_nom(t)=idx(t-sonu)/idx(t-1-sonu)-1`; TL-reel = `r_nom(t) - infl(t)` (TUFE MoM, direktif-literal).
...
bir O+Cagan kararidir; bu harness OLCER + ONERIR, asla otomatik-deploy ETMEZ -- ve burada onerilecek
aday yok.
