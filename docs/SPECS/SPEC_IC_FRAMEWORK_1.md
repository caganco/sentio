...
**Kalan gap'ler (yeni direktif ile kapatılacak):**
...
| `price_limit_hit` her zaman False | Devre kesici (fiyat bandı) tespiti yapılmıyor | G-22 grunt task (kapsam dışı bu direktif) |
...
3. Cagan onaylar → ayrı direktif ile `thresholds.py` güncellenir
...
**Neden:** CB-010 ("statik weight savunulamazlığı"). τ=0 aşamasında (< 60 gün) hiçbir şey değişmez. 60. günden itibaren posterior öneriler raporda görünür, uygulama Cagan onayına bağlı.
...
3. **Cagan onay verir → ayrı Grunt task (`G-22`) ile thresholds.py güncellenir**
4. PR → CI → yeşil → merge
...
## BÖLÜM 3 — Builder Direktif Taslakları (Faz Sırası)

> **Kural:** Aşağıdaki direktifler SPEC onayından önce başlamaz. Merge sırası önemli: D-133 → D-134 → D-135 → D-136.
...
60. günden itibaren üretilen `weight_history.parquet` öneri dosyasıdır; thresholds.py'yi sıfır otomatik değişiklik yapar. Üretime geçmesi için Orchestrator + Cagan onayı (G-22 Grunt task) zorunludur.
...
Mevcut: `MASTER_WEIGHTS["sentiment"] = 0.12`. Bu direktifin kapsamında değiştirilmez. RR-008 (hybrid NLP) implement edildiğinde ayrı direktif + t > 3.0 (`IC_INVESTABLE_TSTAT_MIN` değil, yeni layer için `IC_NEW_LAYER_TSTAT_HURDLE = 3.0` sabiti) uygulanır.
...
## Etkilenen Dosyalar (Toplam — 4 direktif)
...
## Merge Önkoşulları (Her Direktif)
...
**Açık soru (Builder'a):** `IC_INVESTABLE_MONTHS_MIN = 24` çok muhafazakar mı? 2 yıl = ~504 günlük veri birikimi gerekiyor. BIST OS henüz 4 günlük; 2 yıl içinde hiçbir layer "INVEST" statüsüne giremeyecek. `IC_INVESTABLE_MONTHS_MIN = 6` (126 iş günü) daha uygun olabilir. Cagan kararı.

---

*SPEC_IC_FRAMEWORK_1 tamamlandı. Builder direktifleri SPEC onayı bekleniyor.*  
